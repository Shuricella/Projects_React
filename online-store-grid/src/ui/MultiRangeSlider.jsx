...
const MultiRangeSlider = React.forwardRef((props, ref) => {
	...
	const nameSlider = props.nameSlider;
	...
	const [minValue, set_minValue] = useState(parseFloat(props.minValue));
	const [maxValue, set_maxValue] = useState(parseFloat(props.maxValue));
	...
	const triggerInput = (minValue, maxValue) => {
		let retObj = { min, max, minValue, maxValue, nameSlider: nameSlider };
	...
	return (
		<div className={baseClassName} onWheel={onMouseWheel} ref={ref}>
			...
			{label && (
				<div className='label'>
					<div className='label-min'>{min} {unit}</div>
					<div className='label-max'>{max} {unit}</div>
				</div>
			)}
		</div>
	);
